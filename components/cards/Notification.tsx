...
const Notification = ({ image_url, message, text, link_href, create_date }: Props) => {
  ...
  return (
    <Link href={link_href} className="flex justify-between items-center bg-card rounded-md p-2 py-4 border-l-4 border-primary mb-3">
      <div className="flex gap-2 items-center">
        ...
        {!text && <p className="text-foreground font-inter">{message}</p>}
        {text && <p className="text-foreground font-inter">{message} {text && `"${text.length > 40 ? text.slice(0, 40) + "..." : text}"`}</p>}
      </div>
